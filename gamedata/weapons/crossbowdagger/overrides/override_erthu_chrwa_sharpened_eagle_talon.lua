...
function ADVR.onGlobalTick()
	...
	local projectile = game.inventory.currentPrimaryWeapon.AsCrossbowBase().bolt

	if FoundEnemy ~= nil then
		projectile.rb.velocity = vector3.Lerp(projectile.rb.velocity, ((FoundEnemy.transform.position + FoundEnemy.objectCenter) - projectile.transform.position).normalized * 12, 0.3)
	else
